...
原因是 Elasticsearch 在删除快照之前迭代了快照存储桶中的所有文件。当达到 PB 级大小的备份时，此迭代需要很长时间。较新版本的 Elasticsearch 改进了快照代码，大大减少了删除快照的时间。
...
在新集群中，我们使用称为自适应副本选择 （ARS） 的功能。此功能更改搜索分片请求的路由，以便将请求发送到具有给定分片的最不繁忙的数据节点，从而减少搜索时间。在我们的旧集群中，我们实现了类似的功能，我们尝试将搜索请求发送到最不繁忙的可用区。如果对该解决方案感到好奇，我们在较旧的博客文章中对其进行了更多描述。
